              ...
              if(auto* BitCast = dyn_cast<BitCastInst>(GEP->getNextNode())){
                ...
                if(auto* GEPMemStart = dyn_cast<GEPOperator>(BitCast->getNextNode())){
                  if(GEPMemStart->getType() == PointerType::get(PointerType::getInt8PtrTy(Ctx), 0)){
                    ...
                    MemoryStart->addAnnotationMetadata(MemoryStartAnno);
                  }
                }
      ...
      for(auto&Instruction : BasicBlock){
        ...
        if(auto* Load = dyn_cast<LoadInst>(&Instruction)){
          if(auto* GEP = dyn_cast<llvm::Instruction>(Load->getOperand(0))){
            ...
            }else if (isAnnotated(GEP, MemoryStartAnno)){
              Load->addAnnotationMetadata(MemoryStartLoadAnno);
            }
          }
        }
        ...
        if(auto* Store = dyn_cast<StoreInst>(&Instruction)){
          if(auto* StoreDest = dyn_cast<llvm::Instruction>(Store->getOperand(1))){
            if(isAnnotated(StoreDest, MaxMemoryAnno)){
              // Annotate that function should not be optimized, if store to max memory location
              annotateFunction(F);
...
static llvm::RegisterPass<llvm::WasmerFunctionPass> X("wasmer-function", "Hello World Pass",
...
static llvm::RegisterStandardPasses Y(
    ...
    [](const llvm::PassManagerBuilder &Builder,
       llvm::legacy::PassManagerBase &PM) { PM.add(new llvm::WasmerFunctionPass()); });
